...
import { PrayerContext, HadithContext, UIStateContext } from '../components/Mainn';
const Hadith = () => {
  // return <div className="text-white text-4xl">Hadith: ﷺ ﷽</div>;
  // const { seCitie, ciOpen, setCiOpen, prayers, aprayerIndex, bprayerIndex, nDate, aTime, bTime, nTime, hijri, hType, seHType, setSeHType, currH, setCurrH, fHadith, shareText, share, setShare, HFavorite, setFaHadith, faHadith, } = useContext(PrayerContext);
      ...
      <div className="grid grid-cols-1 m-auto ssm:px-10 ssm:m-3 place-items-center ">
        ...
        {fHadith.map(
          (hadith, index) =>
            index === currH && (
              <>
              {faHadith.map((fahadith)=>fahadith.arab === hadith.arab && (hadith.favorie = true)&&HFavorite(index))}
