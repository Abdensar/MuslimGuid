...
import { PrayerContext, AdkarContext, UIStateContext } from '../components/Mainn';
const Adkar = () => {
  // const { seCitie, ciOpen, setCiOpen, prayers, aprayerIndex, bprayerIndex, nDate, aTime, bTime, nTime, hijri, aType, seAType, setSeAType, currI, setCurrI, fAdkar, shareText, share, setShare, AFavorite, setFaAdkar, faAdkar, deviceData } = useContext(PrayerContext);
  const {
    // From PrayerContext
    bTime,
    setCiOpen,
      ...
      <div className="grid grid-cols-1 m-auto ssm:px-10 ssm:m-3 place-items-center ">
        ...
        {fAdkar.map(
          (dikr, index) =>
            index === currI && (
              <>
              {faAdkar.map((faadikr)=>faadikr.content===dikr.content&&(dikr.favorie =true)&&AFavorite(index))}
