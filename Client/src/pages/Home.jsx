...
moment.locale("ar-ma");
const Home = () => {
  // const { seCitie, setSeCitie, ciOpen, setCiOpen, ciValue, setCiValue, prayers, setPrayers, aprayerIndex, bprayerIndex, cities, nDate, aTime, bTime, nTime, hijri, aType, seAType, hType, seHType, shareText, share, setShare, rDikr, rHadith, pick, setPick, alarmI, setAlarmI, pNotClick, setTSound, NotModl, PModel, pickP, setPickP, HFavorite, AFavorite, fAdkar, fHadith, deviceData, updateDevice, } = useContext(PrayerContext);
        ...
        {prayers.map((prayer, index) => (
          <div key={index} className="bg-white rounded-xl shadow-lg h-full">
            {!(deviceData === undefined)
              ? deviceData.prayers.map((dprayer) =>
                  dprayer.key === prayer.key
                    ? ((prayer.not = dprayer.not),
                      (prayer.alarm = dprayer.alarm))
                    : null
                )
              : null}
          ...
          <div className="bg-blue-sidebar ">
            <>
              <div className={` h-fit w-full `}>
                <header className="bg-card-header justify-self-center  grid grid-cols-3 px-1 py-2 text-l h-fit ssm:p-3  ssm:text-3xl font-semibold italic w-full">
                  ...
                  <span className="inline-block justify-self-center text-center place-content-center  text-white">
                    {aType.filter((fAType) =>rDikr.Object.category=== fAType.key).map((ftype) => ftype.name)}
                  </span>
                  <span
              ...
              ${rDikr.Object.reference && `\n[${rDikr.Object.reference}]`} 
              // \n[${aType.filter((fAType) => rDikr.Object.category === fAType.key).name}]
            ...
            <>
              <div className={` h-fit w-full`}>
                <header className="bg-card-header justify-self-center  grid grid-cols-3 px-1 py-2 text-l h-fit ssm:p-3  ssm:text-3xl font-semibold italic w-full">
                  ...
                  <span className="inline-block justify-self-center text-center place-content-center  text-white">
                    {hType.filter((fHType) => rHadith.Object.category === fHType.key).map((ftype) => ftype.name)}
                  </span>
                  <span
                    className="inline-block justify-self-end place-content-center "
                    onClick={() => {
              ...
              {share &&
                shareText(`${rHadith.Object.arab}
                   \n [رواه:${hType.filter((fHType) => rHadith.Object.category === fHType.key
                           ).name
                   }]`) &&
                setShare(false)}
            </>
